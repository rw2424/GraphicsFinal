...
void sphere(float sphereRadius, int sectorCount, int stackCount, std::vector<glm::vec3>& vertex, std::vector<glm::vec3>& normal, std::vector<glm::ivec3>& tria, std::vector<glm::vec2>& uv) {
    // init variables
    vertex.resize(0);
    normal.resize(0);
    tria.resize(0);
    ...
    for (int i = 0; i <= stackCount; ++i) {
        ...
        for (int j = 0; j <= sectorCount; ++j) {
            ...
            // uv coordinates
            glm::vec2 uvVec;
            uvVec[0] = asin(sphereVertexPos.x/sphereRadius)/(float)M_PI + 0.5f;
            uvVec[1] = asin(sphereVertexPos.y/sphereRadius)/(float)M_PI + 0.5f;
            //uvVec[0] = 0.5f - atan2((sphereVertexPos.z/sphereRadius), (sphereVertexPos.x/sphereRadius)) / (2*M_PI);
            //uvVec[1] = -0.5f - asin(sphereVertexPos.y/sphereRadius)/(float)M_PI;
            uv.push_back(uvVec);
        }
    }
...
int main(void)
{
    ...
    bool imageAvailable = loadPPM(image, "../data/Skin_Human_002_NRM.ppm");
    ...
    // Bind the input for UV coordinates
    program.bindVertexAttribArray("vertexUV", UVBuffer);
    ...
    std::ifstream textureFragShader("../shader/textureFragment.glsl");
    std::stringstream textureFragCode;
    textureFragCode << textureFragShader.rdbuf();
    std::ifstream textureVertexShader("../shader/passthroughVertex.glsl");
    std::stringstream textureVertexCode;
    textureVertexCode << textureVertexShader.rdbuf();
    textureProgram.init(textureVertexCode.str(), textureFragCode.str(), "color");
    textureProgram.bind();
    ...
    glTexImage2D(GL_TEXTURE_2D, 0,GL_RGB, 800, 600, 0,GL_RGB, GL_UNSIGNED_BYTE, &image.data[0]);
    ...
    BufferObject textureVBO = VBO;
    BufferObject textureUVBuffer = UVBuffer;

    textureProgram.bindVertexAttribArray("position", textureVBO);
    textureProgram.bindVertexAttribArray("vertexUV", textureUVBuffer);
    */
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(program.uniform("triangleColor"), 1.0f, 0.95f, 0.8f);
        ...
        /*
        // Render to our framebuffer (for render to texture)
        glBindFramebuffer(GL_FRAMEBUFFER, FramebufferName);
        ...
        */
        ...
        /*
		textureProgram.bind();
        glUniformMatrix4fv(textureProgram.uniform("modelMatrix"), 1, GL_FALSE, glm::value_ptr(modelMatrix));
        glUniformMatrix4fv(textureProgram.uniform("viewMatrix"), 1, GL_FALSE, glm::value_ptr(viewMatrix));
        glUniformMatrix4fv(textureProgram.uniform("projMatrix"), 1, GL_FALSE, glm::value_ptr(projMatrix));*/
        // Enable depth test
